...
def load_and_clean_comments(
    ...
    """Đọc, loại bỏ trùng lặp và emoji trong comment_text nếu có chữ, giữ nguyên nếu chỉ có emoji"""
    df_comments = pd.read_csv(comment_path)

    df_comments = df_comments.drop_duplicates(subset=["url", "comment_text"]).copy()

    def remove_emoji_if_text_exists(text: str) -> str:
        if re.search(r"[A-Za-zÀ-ỹ0-9]", text):
            return emoji.replace_emoji(text, replace="")
        return text

    df_comments["comment"] = df_comments["comment_text"].apply(
        remove_emoji_if_text_exists
    )
